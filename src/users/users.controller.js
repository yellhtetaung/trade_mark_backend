...
exports.updateHandler = async (req, res) => {
    ...
    mysql.query(`UPDATE User SET username='${username}', email='${email}', password='${password}', phone_no=${phone_no}, nrc='${nrc}', address='${address}', active=${active === 1}, role='${role}' WHERE id='${id}'`, error => {
        if (error) {
            console.log(error);
            return res.status(500).json({ message: 'Something Wrong!' });
        }
